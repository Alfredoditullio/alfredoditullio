...
/**
 * FolderFloat — vendored from React Bits (reactbits.dev), ported to
 * TypeScript and extended with `onItemHover` so the host can react to the
 * pill under the pointer (we use it to show the matching photo).
 *
 * Local changes vs. upstream are marked with "LOCAL:".
 */
...
type NormItem = { label: string; value: string };
...
export type FolderFloatProps = {
    ...
    /** LOCAL: fires with the hovered pill's index, or null when it leaves. */
    onItemHover?: (index: number | null) => void;
    /** LOCAL: marks one pill as the active/previewed one. */
...
export default function FolderFloat({
    ...
    const latest = useRef<{
        ...
        onItemHover?: FolderFloatProps["onItemHover"];
        drift: number;
        reduce: boolean;
    }>({ drift, reduce: false });
    latest.current = { onSelect, onOpenChange, onItemHover, drift, reduce: latest.current.reduce };
    const popTimer = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
    const liveTimer = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
    const list: NormItem[] = items.map((item) =>
    ...
    useLayoutEffect(() => {
        const measure = () => {
            ...
            const clean = next as Size[];
            setSizes((prev) =>
                prev.length === clean.length &&
                prev.every((s, i) => s.w === clean[i].w && s.h === clean[i].h)
                    ? prev
                    : clean
            );
        };
    ...
    const startPhysics = useCallback(() => {
        ...
        const nodes = els as HTMLButtonElement[];
        const engine = Engine.create({ gravity: { x: 0, y: 0, scale: 0.001 } });
        engine.enableSleeping = false;
        w.engine = engine;
        w.sizes = nodes.map((el) => ({ w: el.offsetWidth, h: el.offsetHeight }));
    ...
    const set = useCallback(
        (next: boolean) => {
            if (!next) {
                stopPhysics();
                // LOCAL: closing the folder clears the preview selection.
                latest.current.onItemHover?.(null);
            }
            setOpen((prev) => {
    ...
    const pick = (item: NormItem, i: number) => {
    ...
    const up = (e: React.PointerEvent<HTMLButtonElement>, i: number, item: NormItem) => {
        const w = world.current;
        const d = w.drag;
        if (!d || d.i !== i || d.id !== e.pointerId) return;
        w.drag = null;
                ...
                {list.map((item, i) => {
                    ...
                    return (
                        <button
                            ...
                            /* LOCAL: surface the hovered pill to the host. */
                            onPointerEnter={() => latest.current.onItemHover?.(i)}
                            onFocus={() => latest.current.onItemHover?.(i)}
                            onClick={(e) => {
